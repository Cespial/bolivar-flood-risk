...
def stack_all_features(
    ...
    region : ee.Geometry, optional
        Analysis region (defaults to Bolivar).
...
def _create_flood_label(region: ee.Geometry) -> ee.Image:
    ...
    Flood pixels (label = 1): JRC occurrence >= 25 % **or** HAND < 10 m
    Non-flood pixels (label = 0): JRC occurrence < 5 % **and** HAND >= 40 m
      **and** slope > 5 degrees

    Thresholds are relaxed compared to Antioquia to account for Bolivar's
    flat terrain and extensive floodplains in the Depresion Momposina.
    ...
    # HAND threshold relaxed to 10 m for flat terrain
    flood = jrc_occ.gte(25).Or(hand.lt(10)).rename("label")

    # Non-flood class: dry areas AND high HAND AND some slope
    # HAND threshold relaxed to 40 m, slope lowered to 5 deg for flat terrain
    non_flood = jrc_occ.lt(5).And(hand.gte(40)).And(slope.gt(5)).rename("label")
...
def generate_training_samples(
    ...
    region : ee.Geometry, optional
        Sampling region (defaults to Bolivar).
...
def run_feature_pipeline(
    ...
    if export and not samples_only:
        ...
        task = export_to_drive(
            image=feature_stack,
            description="bolivar_flood_feature_stack",
            region=region,
            scale=SAMPLE_SCALE,
        )
        tasks.append(task)
    ...
    if export:
        task = export_table_to_drive(
            collection=samples,
            description="bolivar_flood_training_samples",
            file_format="CSV",
        )
        tasks.append(task)
        ...
        task_geo = export_table_to_drive(
            collection=samples,
            description="bolivar_flood_training_samples_geo",
            file_format="GeoJSON",
        )
    ...
    if export and not samples_only:
        ...
        for group_name, img in feature_groups.items():
            task = export_to_drive(
                image=img.toFloat(),
                description=f"bolivar_features_{group_name}",
                region=region,
                scale=SAMPLE_SCALE,
            )
            tasks.append(task)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate flood susceptibility feature stack for Bolivar",
    )
    parser.add_argument(
        "--no-export",
        action="store_true",
